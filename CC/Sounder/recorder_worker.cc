...
void RecorderWorker::init(void)
{
    ...
    this->hdf5_->write_attribute(
        "DATA_SUBCARRIER_NUM", this->cfg_->symbol_data_subcarrier_num());
    ...
    this->hdf5_->write_attribute(
        "BS_CH_PER_RADIO", this->cfg_->bs_channel().length());
    ...
    this->hdf5_->write_attribute(
        "BS_BEAMSWEEP", this->cfg_->beam_sweep() ? 1 : 0);
    ...
    // Number of Pilots
    this->hdf5_->write_attribute(
        "PILOT_NUM", this->cfg_->pilot_slot_per_frame());
    ...
    if (this->cfg_->pilot_sc_ind().size() > 0)
        this->hdf5_->write_attribute(
            "OFDM_PILOT_SC", this->cfg_->pilot_sc_ind());
    if (this->cfg_->pilot_sc().size() > 0)
        this->hdf5_->write_attribute(
            "OFDM_PILOT_SC_VALS", this->cfg_->pilot_sc());
    ...
    if (this->cfg_->internal_measurement() == false) {
        ...
        this->hdf5_->write_attribute(
            "CL_CH_PER_RADIO", this->cfg_->cl_sdr_ch());

        // Client AGC enable flag
        this->hdf5_->write_attribute(
            "CL_AGC_EN", this->cfg_->cl_agc_en() ? 1 : 0);

        // RX Gain RF channel A
        this->hdf5_->write_attribute(
            "CL_RX_GAIN_A", this->cfg_->cl_rxgain_vec().at(0));

        // TX Gain RF channel A
        this->hdf5_->write_attribute(
            "CL_TX_GAIN_A", this->cfg_->cl_txgain_vec().at(0));

        // RX Gain RF channel B
        this->hdf5_->write_attribute(
            "CL_RX_GAIN_B", this->cfg_->cl_rxgain_vec().at(1));

        // TX Gain RF channel B
        this->hdf5_->write_attribute(
            "CL_TX_GAIN_B", this->cfg_->cl_txgain_vec().at(1));
    ...
    if (this->cfg_->ul_data_slot_present()) {
        ...
        this->hdf5_->write_attribute(
            "UL_DATA_FRAME_NUM", this->cfg_->ul_data_frame_num());

        // Names of Files including uplink tx frequency-domain data
        if (this->cfg_->ul_tx_fd_data_files().size() > 0) {
            this->hdf5_->write_attribute(
                "TX_FD_DATA_FILENAMES", this->cfg_->ul_tx_fd_data_files());
        }
        ...
        for (size_t i = 0; i < this->cfg_->txdata_freq_dom().size(); i++) {
            ...
            this->hdf5_->write_attribute(
                var.c_str(), this->cfg_->txdata_freq_dom().at(i));
        }
        ...
        for (size_t i = 0; i < this->cfg_->txdata_time_dom().size(); i++) {
            ...
            this->hdf5_->write_attribute(
                var.c_str(), this->cfg_->txdata_time_dom().at(i));
        }
    }
    // ********************* //
    ...
    if (this->cfg_->cl_rx_thread_num() > 0
        ...
        std::array<hsize_t, kDsDimsNum> dims_dl_data = { MAX_FRAME_INC,
...
void RecorderWorker::record(int tid, Package* pkg)
{
    ...
    if ((this->cfg_->max_frame()) != 0
        ...
        if (pkg->frame_id >= this->max_frame_number_) {
            ...
            this->hdf5_->closeDataset();
            this->hdf5_->openDataset();
            this->hdf5_->setTargetPrimaryDimSize(
