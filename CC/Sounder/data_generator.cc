...
void DataGenerator::GenerateData(const std::string& directory)
{
                ...
                for (size_t u = 0; u < cfg_->cl_ul_slots().at(i).size(); u++) {
                    for (size_t h = 0; h < cfg_->cl_sdr_ch(); h++) {
                        ...
                        for (size_t s = 0; s < cfg_->symbol_per_slot(); s++) {
                            ...
                            for (size_t c = 0; c < cfg_->data_ind().size();
                                 c++) {
                                data_bits.push_back(
                                    (uint8_t)(rand() % mod_order));
                            }
                            std::fwrite(data_bits.data(),
                                cfg_->symbol_data_subcarrier_num(),
                            ...
                            for (size_t c = 0; c < cfg_->pilot_sc().size();
                                 c++) {
                                sc = cfg_->pilot_sc_ind().at(c);
                                ofdm_sym[sc] = cfg_->pilot_sc().at(c);
                            }
                            auto tx_sym
                                = CommsLib::IFFT(ofdm_sym, cfg_->fft_size(),
                                    1.f / cfg_->fft_size(), false);
                            tx_sym.insert(tx_sym.begin(),
                                tx_sym.end() - cfg_->cp_size(),
                                tx_sym.end()); // add CP
                            data_time_dom.insert(data_time_dom.end(),
                                tx_sym.begin(), tx_sym.end());
                        ...
                        std::fwrite(data_time_dom_ci16.data(),
                            cfg_->samps_per_slot(), sizeof(int16_t) * 2,
                            fp_tx_t);
                    }
                }
            }
    ...
    if (cfg_->dl_slot_per_frame() > 0) {
        int mod_type = cfg_->data_mod() == "64QAM"
            ? CommsLib::QAM64
        ...
        for (size_t i = 0; i < cfg_->num_bs_sdrs_all(); i++) {
            std::string filename_tag = cfg_->data_mod() + "_"
                ...
                + std::to_string(cfg_->dl_data_frame_num()) + "_"
                + cfg_->bs_channel() + "_" + std::to_string(i) + ".bin";

            std::string filename_dl_data_b
                = directory + "/dl_data_b_" + filename_tag;
            std::printf(
                "Saving DL data bits to %s\n", filename_dl_data_b.c_str());
