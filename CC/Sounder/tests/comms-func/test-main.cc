...
int main() {
  ...
  for (size_t i = 0; i < testAbs0.size(); i++) {
    ...
    if (std::abs(testAbs0[i] - gt) > 0.01) pass = false;
  }
  ...
  for (size_t i = 0; i < testMultRes.size(); i++) {
    if ((int)i < dly)
      std::cout << "Float Op Result: " << testMultRes[i] << ", INT Op Result: ("
      ...
      std::cout << "Float Op Result: " << testMultRes[i] << ", INT Op Result: ("
                ...
                << testDataCF32[i] * (conj ? std::conj(testDataCF32[i - dly])
                                           : testDataCF32[i - dly])
                << std::endl;
  }
  ...
  std::vector<std::complex<float>> testDataExtend(testCorrSeqCF32.size() - 1,
                                                  0);
  testDataExtend.insert(testDataExtend.end(), testDataCF32.begin(),
                        testDataCF32.end());
  ...
  testDataExtend1.insert(testDataExtend1.end(), testAbs0.begin(),
                         testAbs0.end());
  ...
  int sync_index = CommsLib::find_beacon_avx(buffs, gold_sym_orig, 1.f);
  ...
  for (size_t i = 0; i < num_frames; i++) {
    ...
    int sync_index = CommsLib::find_beacon_avx(buff0, gold_sym_orig, 1.f);
    if (sync_index >= 0 && sync_index < samps_per_frame) index[sync_index]++;
    printf("Frame: %zu, sync_idx: %zu, total_idx: %d\n", i, sync_index,
           (sync_index + i * samps_per_frame));
  }
  for (size_t i = 0; i < samps_per_frame; i++) {
    if (index[i] > 0) printf("SYNC_INDEX %d: %zu times\n", index[i], i);
  }
  fclose(fp);
#endif
#endif
