    ...
    std::array<hsize_t, kDsDimsNum> chunk_dims)
{
    ...
    try {
        ...
        this->file_->createDataSet(
            ds_name, H5::PredType::STD_I16BE, ds_dataspace, ds_prop);
        ds_prop.close();
    }
    ...
    if (this->file_ != nullptr) {
        for (size_t i = 0; i < dataset_str_.size(); i++) {
            ...
            try {
                H5::Exception::dontPrint();
                this->extendDataset(
                    dataset_str_.at(i), this->target_prim_dim_size);
    ...
    std::array<hsize_t, kDsDimsNum> wrt_dim, short* wrt_data)
{
    ...
    try {
        ...
        this->datasets_.at(ds_id)->write(
            wrt_data, H5::PredType::NATIVE_INT16, memspace, filespace);
        filespace.close();
    }
    // catch failure caused by the DataSet operations
    catch (H5::DataSetIException& error) {
...
void Hdf5Lib::write_attribute(const char name[], double val)
{
    ...
    H5::Attribute att = this->group_.createAttribute(
        name, H5::PredType::NATIVE_DOUBLE, attr_ds);
    att.write(H5::PredType::NATIVE_DOUBLE, &val);
}

void Hdf5Lib::write_attribute(const char name[], const std::vector<double>& val)
{
...
void Hdf5Lib::write_attribute(
    const char name[], const std::vector<std::complex<int16_t>>& val)
{
    size_t size = val.size();
    hsize_t dims[] = { 2 * size };
    H5::DataSpace attr_ds = H5::DataSpace(1, dims);
...
void Hdf5Lib::write_attribute(
    const char name[], const std::vector<std::complex<float>>& val)
{
    size_t size = val.size();
    hsize_t dims[] = { 2 * size };
    H5::DataSpace attr_ds = H5::DataSpace(1, dims);
...
void Hdf5Lib::write_attribute(const char name[], const std::vector<size_t>& val)
{
    size_t size = val.size();
    hsize_t dims[] = { size };
    H5::DataSpace attr_ds = H5::DataSpace(1, dims);
...
void Hdf5Lib::write_attribute(const char name[], const std::string& val)
{
    ...
    H5::Attribute att
        = this->group_.createAttribute(name, strdatatype, attr_ds);
    att.write(strdatatype, val);
}

void Hdf5Lib::write_attribute(
    const char name[], const std::vector<std::string>& val)
{
    if (val.empty())
        return;
    size_t size = val.size();
